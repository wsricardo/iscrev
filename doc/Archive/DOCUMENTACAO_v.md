...
> 
> Para a documentação atualizada, consulte os arquivos no diretório `src/` do projeto.
...
│   ├── Google Fonts — 4 famílias via CDN assíncrono
│   ├── KaTeX CSS — cdn.jsdelivr.net
│   └── KaTeX JS  — cdn.jsdelivr.net  ← SÍNCRONO (sem defer/async)
│
├── <style>  (linhas 16–430)
│   ├── Reset universal (box-sizing, margin, padding)
│   ├── Tokens de design (:root com CSS Custom Properties)
│   ├── Base (html, body, .app)
│   ├── Sidebar e seus filhos
│   ├── Main e Editor
│   ├── Toolbar principal + Pen toolbar
│   ├── Editor wrap, editor-area, notebook-bg
│   ├── #entry-raw, #entry-preview, #pen-svg
│   ├── Header controls (lang-switcher, btn-fullscreen)
│   ├── Toast, Equação dialog, Welcome screen
│   ├── @media print
...
│   └── .eq-overlay
│
└── <script>  (linhas 781–2731)
    └── IIFE com 'use strict' — escopo isolado
        ├── SEÇÃO 0  — i18n
...
**Por que uma IIFE?**  
Todo o JS está em `(function(){ 'use strict'; ... })()`. Isso cria escopo privado — nenhuma variável polui `window`, evitando conflito com scripts externos como o KaTeX que escreve `window.katex`.

**Por que KaTeX é síncrono?**  
`mdToHtml()` chama `katex.renderToString()` diretamente, de forma síncrona. Se o script fosse `defer`, o usuário poderia clicar em Preview antes do KaTeX carregar e receber erro. O carregamento síncrono bloqueia o parser HTML até o `window.katex` estar disponível, garantindo que a IIFE encontre a biblioteca pronta.
